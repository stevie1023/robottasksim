...
class ReachTask(Task):
    ...
    def __init__(self, mujoco_arena, mujoco_robot, mujoco_objects, initializer=None):
        ...
        self.merge_objects2(mujoco_objects)
    ...
    def merge_objects2(self, mujoco_objects):
        """Adds physical objects to the MJCF model."""
        self.mujoco_objects = mujoco_objects
        self.objects = []  # xml manifestation
        self.targets = []  # xml manifestation
        ...
        for obj_name, obj_mjcf in mujoco_objects.items():
            self.merge_asset(obj_mjcf)
            # Load object
            obj = obj_mjcf.get_collision(name=obj_name+'_site', site=True)
            obj.append(new_site(name=obj_name))
            self.objects.append(obj)
            self.worldbody.append(obj)
